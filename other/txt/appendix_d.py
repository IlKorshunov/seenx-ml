...
def aggregate(video_path: str, audio_path: str, output_path: str,
    ...
    def add_to_accumulated(mapped: pd.DataFrame) -> None:
        """Добавляет новые столбцы; при наличии параметра only дополнительно
    ...
    # Визуальные признаки на основе предобученных моделей кадрового анализа
    ...
    _STEPS = [
        ...
        ("semantic embeddings",
         ...
         lambda: extract_semantic_embeddings(video_path, config, existing)),
...
def aggregate_batch(data_dir: str, output_dir: str, config: Config):
    ...
    Для каждой группы извлекателей последовательно обрабатываются все ролики,
    после чего нейросетевые модели группы выгружаются из памяти. Это позволяет
    загружать каждую тяжёлую модель в видеопамять однократно за весь прогон,
    ...
    for group_name, group_cols, vram_est in _BATCH_GROUPS:
        if not any(group_cols - existing for _, existing in video_data.values()): сontinue
        for v in videos:
