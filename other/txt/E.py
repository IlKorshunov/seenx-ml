...
def add_common_args(parser: argparse.ArgumentParser) -> ar-gparse.ArgumentParser:
...
def load_and_filter_data(args):
    """Загрузка посекундных табличных представлений всех ви-деороликов
    ...
    output_video_ids = re-solve_output_video_ids(args.output_dir_features, video_dfs)
...
def resolve_split(args, video_ids, output_video_ids):
    ...
    train_ids, val_ids = resolve_train_val_split(args, video_ids, out-put_video_ids)
    return apply_train_id_file_filter(train_ids, args), val_ids


def make_normalizer(args, video_dfs, train_ids, feature_cols):
    """Вычисление нормализующих статистик на обучающей выбор-ке."""
...
def compute_baseline_curve(video_dfs, train_ids, normalizer):
    ...
    for v in train_ids:
        ret = pd.to_numeric(video_dfs[v]["retention"], er-rors="coerce").fillna(0).values
        acc[: len(ret)] += ret
        cnt[: len(ret)] += 1.0
...
def predict_all_videos(*, video_ids, val_ids, video_dfs, predict_fn,
    ...
    """Инференс по всем роликам выборки и вычисление метрик ка-чества."""
...
def save_metrics_json(args, *, model_name, feature_cols, n_feat,
    ...
    Path(os.path.join(args.output_dir, "metrics.json")).write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encod-ing="utf-8"
    )
...
def main():
    ...
    # Подготовка данных: загрузка, фильтрация, разбиение, норма-лизация
    ...
    normalizer, ref_sec, video_weights = make_normalizer(args, vid-eo_dfs, train_ids, feature_cols)

    # Построение датасетов  и инициализация модели
    train_ds = MultimodalWindowedDataset(video_dfs, vid-eo_embeddings, train_ids,
                                          feature_cols, normalizer,
                                          args.window_size, args.window_stride,
                                          video_weights=video_weights)
    val_ds = MultimodalWindowedDataset(video_dfs, vid-eo_embeddings, val_ids,
                                        feature_cols, normalizer,
                                        args.window_size, args.window_stride)
    train_dl = DataLoader(train_ds, batch_size=args.batch_size, shuf-fle=True)
    val_dl = DataLoader(val_ds, batch_size=args.batch_size, shuf-fle=False)
    ...
    # Сохранение полного состояния эксперимента
    save_metrics_json(args, mod-el_name=f"Multimodal{args.arch.title()}",
