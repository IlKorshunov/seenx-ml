...
COMMENTS_ROOT = _PROJECT_ROOT / "src" / "get_data" / "comments"
...
def load_youtube():
    if not TOKEN_PATH.is_file():
        raise FileNotFoundError(f"Нет {TOKEN_PATH}. python src/get_data/auth.py")
...
def main():
    ...
    ap.add_argument("--playlist-map-out", type=Path, default=None, help="Путь для JSON карты (по умолчанию src/get_data/comments/video_playlist_membership.json)")
    args = ap.parse_args()
